...
// Константы, определяющие время для функции задержки
.equ CycleIn25ms, 0xCF6F
.equ CycleIn2500us, 0x19ED
.equ CycleIn500us, 0x52F

.equ PeriodInCycle, CycleIn25ms
.equ TimeOnMinInCycle, CycleIn500us
.equ TimeOnMaxInCycle, CycleIn2500us

// Шаг изменения ШИМ сигнала для сервопривода
.equ StepServo, 10
...
Reset_Handler:
    ...
    str    r1,    [r0]                                // Загружаем это значение регистр

    // Конфигурация порта PC5
    ldr    r0,    =GPIOC_CRL                          // Загружаем адрес регистра конфигурации GPIOC low
    mov    r1,    #0x03                               // 4-битная маска настроек для Output mode 50mHz, Push-Pull (0x03 = 0b0011)
    ldr    r2,    [r0]                                // Считываем содержимое регистра конфигурации GPIOC low
    bfi    r2,    r1,    #20,    #4                   // Копируем биты маски в позицию, соответствующую PIN5
    str    r2,    [r0]                                // Загружаем это значение регистр

    // Регистр с 0 для операций сравления
    mov    r11,   #0

    // Инициализация переменных
    ldr    r3,    =TimeOnMaxInCycle                   // r3  = 2,5 ms
    ldr    r4,    =TimeOnMinInCycle                   // r4 = 0,5 ms
    ldr    r5,    =PeriodInCycle                      // Период
    mov    r6,    #0                                  // FlagDirection

    ldr    r7,    =TimeOnMinInCycle                   // Ton = 0,5 ms
    sub    r8,    r5,    r7                           // Toff = Period - Ton

    ldr    r9,    =GPIOC_BSRR                         // Адрес регистра порта выходных сигналов PC

// Бесконечный цикл
loop:
// Генерация ШИМ сигнала сервопривода
	// Устанавливаем порт PC5 в On
...
// Рассчет параметров ШИМ сигнала
	// Проверка, что текущее знаечние Ton меньше максимального, иначе смена напрвления
    cmp    r3,    r7                                  // if (TimeOnMaxInCycle < r7) {r6 = 1;}
    it     LO
    movlo  r6,    #1

	// Проверка, что текущее знаечние Ton больше минимального, иначе смена напрвления
    cmp    r4,    r7                                  // if (TimeOnMinInCycle > r7) {r6 = 0;}
    it     GT
    movgt  r6,    #0

	// В зависимости от флага увеличиваем или уменьшаем текущее значение Ton
    cmp    r11,   r6                                  // r6 = 0, r11 - флаг
    it     EQ
    addeq  r7,    r7,    #StepServo                   // Ton++;
    it     NE
    subne  r7,    r7,    #StepServo                   // Ton--;

	// Рассчет Toff = Period - Ton
    sub    r8,    r5,    r7
...
delay:
    ...
    it     NE
    bne    delay                                      // Переход на метку delay, если Z==0 (результат вычитания не равен нулю)
    bx     lr                                         // Выход из подпрограммы (переход к адресу в регистре LR)
